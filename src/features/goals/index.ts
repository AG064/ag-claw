...
class GoalsFeature implements FeatureModule {
  ...
  private config: { enabled: boolean; dbPath: string } = {
    enabled: false,
  ...
  async start(): Promise<void> {
    const goalCount = (this.db.prepare('SELECT COUNT(*) as c FROM goals').get() as { c: number }).c;
...
export default new GoalsFeature();
